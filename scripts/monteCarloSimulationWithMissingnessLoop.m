...
missing_iters = 7; %.0 to .9 = 1 to 10
max_mi = .7;
...
for j=1:r
    ...
    hold on;
    plot(missingnesses,mse_by_mprob_nearcorr,'Color','blue');
    plot(missingnesses,mse_by_mprob_no_proj,'Color','red');
    ...
    for b=(a+1):num_metabolites
        ...
        for j=1:r
            ...
            p=plot(missingnesses*100,mse_by_mprob_nearcorr,'Color','blue',...
                'LineWidth',.02);
            p.Color(4) = .05;
            % p=plot(missingnesses,mse_by_mprob_no_proj,'Color','red',...
            %     'LineWidth',.02);
            % p.Color(4) = .5;
            % xlim([0,.6]);
            % ylim([-.05,.05]);
